...
max_error = [-float('inf')] * 6
min_error = [float('inf')] * 6
pose_cnt = 0
...
for i, (topic, msg, t) in enumerate(bag.read_messages(topics=[pid_topic, flight_state_topic])):
    ...
    if topic == flight_state_topic:
        ...
        if msg.state == LAND_STATE:
            ...
            print("Maximum value of pos errors: [{}, {}, {}], Minimum value of pos errors: [{}, {}, {}]".format(max_error[0], max_error[1], max_error[2], min_error[0], min_error[1], min_error[2]))
            print("Maximum value of att errors: [{}, {}, {}], Minimum value of att errors: [{}, {}, {}]".format(max_error[3], max_error[4], max_error[5], min_error[3], min_error[4], min_error[5]))
    ...
    if topic == pid_topic:
        ...
        if t.to_sec() > start_t + offset_t:
            ...
            max_error[0] = max(max_error[0], msg.x.pos_error)
            max_error[1] = max(max_error[1], msg.y.pos_error)
            max_error[2] = max(max_error[2], msg.z.pos_error)
            max_error[3] = max(max_error[3], msg.roll.pos_error)
            max_error[4] = max(max_error[4], msg.pitch.pos_error)
            max_error[5] = max(max_error[5], msg.yaw.pos_error)

            min_error[0] = min(min_error[0], msg.x.pos_error)
            min_error[1] = min(min_error[1], msg.y.pos_error)
            min_error[2] = min(min_error[2], msg.z.pos_error)
            min_error[3] = min(min_error[3], msg.roll.pos_error)
            min_error[4] = min(min_error[4], msg.pitch.pos_error)
            min_error[5] = min(min_error[5], msg.yaw.pos_error)
